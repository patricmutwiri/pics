...
/**
 * View class for a list of Ordersplaced.
 *
 * @since  1.6
 */
class OrdersViewOrdersplaced extends JViewLegacy
{
	protected $items;
	...
	public function display($tpl = null)
	{
		...
		OrdersHelpersOrders::addSubmenu('ordersplaced');
	...
	protected function addToolbar()
	{
		...
		JToolBarHelper::title(JText::_('COM_ORDERS_TITLE_ORDERS'), 'ordersplaced.png');
		...
		if (file_exists($formPath))
		{
			...
			{
				JToolBarHelper::addNew('ordersplaced.add', 'JTOOLBAR_NEW');
				JToolbarHelper::custom('ordersplaced.duplicate', 'copy.png', 'copy_f2.png', 'JTOOLBAR_DUPLICATE', true);
			}

			if ($canDo->get('core.edit') && isset($this->items[0]))
			{
				JToolBarHelper::editList('ordersplaced.edit', 'JTOOLBAR_EDIT');
			}
		}
		...
		if ($canDo->get('core.edit.state'))
		{
			if (isset($this->items[0]->state))
			{
				...
				JToolBarHelper::custom('ordersplaced.publish', 'publish.png', 'publish_f2.png', 'JTOOLBAR_PUBLISH', true);
				JToolBarHelper::custom('ordersplaced.unpublish', 'unpublish.png', 'unpublish_f2.png', 'JTOOLBAR_UNPUBLISH', true);
			}
			elseif (isset($this->items[0]))
			{
				// If this component does not use state then show a direct delete button as we can not trash
				JToolBarHelper::deleteList('', 'ordersplaced.delete', 'JTOOLBAR_DELETE');
			}

			if (isset($this->items[0]->state))
			{
				JToolBarHelper::divider();
				JToolBarHelper::archiveList('ordersplaced.archive', 'JTOOLBAR_ARCHIVE');
			}

			if (isset($this->items[0]->checked_out))
			{
				JToolBarHelper::custom('ordersplaced.checkin', 'checkin.png', 'checkin_f2.png', 'JTOOLBAR_CHECKIN', true);
			}
		}
		...
		if (isset($this->items[0]->state))
		{
			...
			{
				JToolBarHelper::deleteList('', 'ordersplaced.delete', 'JTOOLBAR_EMPTY_TRASH');
				JToolBarHelper::divider();
			}
			elseif ($canDo->get('core.edit.state'))
			{
				JToolBarHelper::trash('ordersplaced.trash', 'JTOOLBAR_TRASH');
				JToolBarHelper::divider();
			}
		}
