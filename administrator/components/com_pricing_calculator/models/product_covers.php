...
/**
 * List Model for product_covers.
 *
 * @package     Pricing_calculator
 * @subpackage  Models
 */
class Pricing_calculatorModelProduct_covers extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'a.publish_down', 'publish_down','ordering', 'state', 'cover_name'
			);
		}
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__product_covers AS a');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('a.cover_name LIKE' . $s );
			}
		}
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__product_covers AS a ON a.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
