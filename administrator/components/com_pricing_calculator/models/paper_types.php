...
/**
 * List Model for paper_types.
 *
 * @package     Pricing_calculator
 * @subpackage  Models
 */
class Pricing_calculatorModelPaper_types extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'a.publish_down', 'publish_down','ordering', 'state', 'paper_name', 'paper_description'
			);
		}
	 ...
	 * @return  void
	 */
	protected function populateState($ordering = 'paper_name', $direction = 'ASC')
	{
		// Get the Application
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__paper_types AS a');
		...
		if (!empty($search))
		{
			...
			elseif (stripos($search, 'paper_name:') === 0)
			{
				$search = $db->quote('%' . $db->escape(substr($search, strlen('paper_name:')), true) . '%');
				$query->where('(a.paper_name LIKE ' . $search);
			}
			...
			else
			{
				...
				$query->where('a.paper_name LIKE' . $s . ' OR a.paper_description LIKE' . $s );
			}
		}
		...
		$sort = $this->getState('list.ordering', 'paper_name');
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__paper_types AS a ON a.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
