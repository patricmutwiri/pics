...
class Pricing_calculatorViewPricings extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		Pricing_calculatorHelper::addSubmenu('pricings');
	...
	protected function addToolbar()
	{
		...
		JToolBarHelper::title(JText::_('COM_PRICING_CALCULATOR_PRICING_VIEW_PRICINGS_TITLE'));
		
		if ($canDo->get('core.create') || (count($user->getAuthorisedCategories('com_pricing_calculator', 'core.create'))) > 0 )
		{
			JToolBarHelper::addNew('pricing.add','JTOOLBAR_NEW');
		}

		if (($canDo->get('core.edit') || $canDo->get('core.edit.own')) && isset($this->items[0]))
		{
			JToolBarHelper::editList('pricing.edit','JTOOLBAR_EDIT');
		}
		...
		if ($canDo->get('core.edit.state'))
		{
            if (isset($this->items[0]->published))
			{
				...
				JToolbarHelper::publish('pricings.publish', 'JTOOLBAR_PUBLISH', true);
				JToolbarHelper::unpublish('pricings.unpublish', 'JTOOLBAR_UNPUBLISH', true);
            } 
			else if (isset($this->items[0]))
			{
                // Show a direct delete button
                JToolBarHelper::deleteList('', 'pricings.delete','JTOOLBAR_DELETE');
            }

            if (isset($this->items[0]->published))
			{
			    JToolBarHelper::divider();
			    JToolBarHelper::archiveList('pricings.archive','JTOOLBAR_ARCHIVE');
            }
            
			if (isset($this->items[0]->checked_out))
			{
				JToolbarHelper::checkin('pricings.checkin');
            }
		}
        ...
        if (isset($this->items[0]->published))
		{
			...
			{
			    JToolBarHelper::deleteList('', 'pricings.delete','JTOOLBAR_EMPTY_TRASH');
			    JToolBarHelper::divider();
		    }
			else if ($state->get('filter.published') != -2 && $canDo->get('core.edit.state'))
			{
			    JToolBarHelper::trash('pricings.trash','JTOOLBAR_TRASH');
			    JToolBarHelper::divider();
		    }
        }
