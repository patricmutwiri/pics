 ...
 * @subpackage  Models
 */
class Perfectpics_productsModelPerfectPics_Products extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'a.publish_down', 'publish_down','ordering', 'state', 'product_name', 'price_from'
			);
		}
	 ...
	 * @return  void
	 */
	protected function populateState($ordering = 'product_name', $direction = 'ASC')
	{
		// Get the Application
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__perfectpics_product AS a');
		...
		if (!empty($search))
		{
			...
			elseif (stripos($search, 'product_name:') === 0)
			{
				$search = $db->quote('%' . $db->escape(substr($search, strlen('product_name:')), true) . '%');
			...
			else
			{
				...
				$query->where('a.product_name LIKE' . $s . ' OR a.price_from LIKE' . $s );
			}
		}
		...
		$sort = $this->getState('list.ordering', 'product_name');
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__perfectpics_product AS a ON a.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
