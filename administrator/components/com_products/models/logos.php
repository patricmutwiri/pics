...
/**
 * List Model for logos.
 *
 * @package     Products
 * @subpackage  Models
 */
class ProductsModelLogos extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'a.publish_down', 'publish_down','ordering', 'state', 'title', 'details', 'logo_image'
			);
		}
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__logos AS a');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('a.title LIKE' . $s . ' OR a.details LIKE' . $s . ' OR a.logo_image LIKE' . $s );
			}
		}
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__logos AS a ON a.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
