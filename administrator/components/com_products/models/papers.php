...
/**
 * List Model for papers.
 *
 * @package     Products
 * @subpackage  Models
 */
class ProductsModelPapers extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'a.publish_down', 'publish_down','ordering', 'state', 'title', 'description', 'categories', 'product_highlight', 'paper_image', 'paper_price'
			);
		}
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__papers AS a');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('a.title LIKE' . $s . ' OR a.description LIKE' . $s . ' OR a.categories LIKE' . $s . ' OR a.product_highlight LIKE' . $s . ' OR a.paper_image LIKE' . $s . ' OR a.paper_price LIKE' . $s );
			}
		}
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__papers AS a ON a.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
