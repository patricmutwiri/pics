...
/**
 * List Model for end_sheets.
 *
 * @package     Products
 * @subpackage  Models
 */
class ProductsModelEnd_sheets extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'a.publish_down', 'publish_down','ordering', 'state', 'title', 'description', 'price', 'product_highlight', 'categories', 'end_sheet_image'
			);
		}
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__end_sheets AS a');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('a.title LIKE' . $s . ' OR a.description LIKE' . $s . ' OR a.price LIKE' . $s . ' OR a.product_highlight LIKE' . $s . ' OR a.categories LIKE' . $s . ' OR a.end_sheet_image LIKE' . $s );
			}
		}
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__end_sheets AS a ON a.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
