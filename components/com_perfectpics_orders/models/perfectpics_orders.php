...
/**
 * Item Model for perfectpics_orders.
 *
 * @package     Perfectpics_orders
 * @subpackage  Models
 */
class Perfectpics_ordersModelPerfectPics_Orders extends JModelAdmin
{
	/**
	 * @var        string    The prefix to use with controller messages.
	 * @since   1.6
	 */
	protected $text_prefix = 'COM_PERFECTPICS_ORDERS';
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_perfectpics_orders.perfectpics_orders';
	 ...
	 */
	public function getTable($type = 'PerfectPics_Orders', $prefix = 'Perfectpics_ordersTable', $config = array())
	{
	...
	{
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('PerfectPics_Orders.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$query = $db->getQuery(true)
					->select('a.*')
					->from('#__perfectpics_orders AS a');
				...
				if (empty($data))
				{
					JError::raiseError(404, JText::_('COM_PERFECTPICS_ORDERS_ERROR_FEED_NOT_FOUND'));
				}
