 ...
 * @subpackage  Views
 */
class ProductsViewCategory extends JViewCategory
{
	/**
	...
	protected function prepareDocument()
	{
		...
		if ($menu && ($menu->query['option'] != 'com_products' || $menu->query['view'] == 'logo' || $id != $this->category->id))
		{
			$path = array(array('title' => $this->category->title, 'link' => ''));
			$category = $this->category->getParent();

			while (($menu->query['option'] != 'com_products' || $menu->query['view'] == 'logo' || $id != $category->id) && $category->id > 1)
			{
