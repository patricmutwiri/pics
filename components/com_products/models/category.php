...
class ProductsModelCategory extends JModelList
{
	...
	/**
	 * The list of other products categories.
	 *
	 * @access    protected
	 * @var        array
	 */
	...
	protected function getListQuery()
	{
		...
		$query->select($this->getState('list.select', 'a.*'))
			->from($db->quoteName('#__logos') . ' AS a');
	...
	/**
	 * Build a list of covers from cats
	 *
	 * @return  array
	 *
	 * @since   1.6
	...
	public function getCovers()
	{
		...
		$db 	= $this->getDbo();
		$query 	= $db->getQuery(true);
		$query 	= $query->select('*, a.id as aid, a.alias as product_alias')
					->from('#__covers AS a')
					->where($db->quoteName('a.categories') .' = '.$db->quote($id));
		// Filter by published state.
		$query->select('c.title AS category_title, c.path AS category_route, c.access AS category_access, c.alias AS category_alias')
			->join('LEFT', '#__categories AS c ON c.id = a.categories');
