...
class Pricing_calculatorModelPricings extends JModelList
{
	...
	public function getCovers()
	{
		$db = $this->getDbo();
		$query = $db->getQuery(true);
		$query = $query->select('id,cover_name,price')->from('#__product_covers')->where($db->quoteName('published') .' = 1');
		$db->setQuery($query);
		$covers = $db->loadObjectList();
	...
	/**
	 * Build a list of product paper types
	 *
	 * @return  array
	 *
	 * @since   1.6
	*/
	public function getPapertypes($id=0)
	{
		$db 	= $this->getDbo();
		$query 	= $db->getQuery(true);
		$query 	= $query->select('id,paper_name,paper_description')->from('#__paper_types');
					if(!empty($id)) {
						$query->where('id = '.(int)$id);
					}
					$query->where('published = 1');
		$db->setQuery($query);
		$papertypes = $db->loadObjectList();
		if(!empty($papertypes)) {
			return $papertypes;
		} else {
			die('Empty TYPES !');
	...
	/**
	 * Build a list of product grouped by name
	 *
	 * @return  array
	 *
	 * @since   1.6
	*/
	public function getGroupedProducts($catid)
	{
		$db 	= $this->getDbo();
		$query 	= $db->getQuery(true);
		$query 	= $query->select('*')->from('#__perfectpics_product');
				  $query->where('published = 1');
	...
	/**
	 * Build a list of sizes grouped by size
	 *
	 * @return  array
	 *
	 * @since   1.6
	*/
	public function getPsizes($catid)
	{
		$db 	= $this->getDbo();
		$query 	= $db->getQuery(true);
		$query 	= $query->select('DISTINCT `range`')->from('#__perfectpics_product');
				  $query->where('published = 1');
				  $query->where('catid = '.(int)$catid.'');
				  //$query->group($db->quoteName('range'));
		$db->setQuery($query);
	...
	*/
	public function getSizes($id=0)
	{
		$db 	= $this->getDbo();
		$query 	= $db->getQuery(true);
		$query 	= $query->select('id,`range`')->from('#__perfectpics_product');
					if(!empty($id)) {
						$query->where('papertype = '.(int)$id);
					}
					$query->where('published = 1');
		$db->setQuery($query);
		$sizes = $db->loadObjectList();
	...
	/**
	 * Build a list of products with this papertype n category
	 *
	 * @return  array
	 *
	 * @since   1.6
	*/
	public function getPaperProducts($paperid=0,$catid,$size,$pname)
	{
		$db 	= $this->getDbo();
		$query 	= $db->getQuery(true);
		$query 	= $query->select('*')
					->from('#__perfectpics_product')
					->where('papertype = '.(int)$paperid)
					->where('catid = '.(int)$catid)
					->where($db->quoteName('range') .' = '.$db->quote($size).'')
					->where('product_name = '.$db->quote($pname).'')
					->where('published = 1');
		$db->setQuery($query);
		$products = $db->loadObjectList();
		return $products;
