	...
	<div class="page-header">
		...
		<h2 itemprop="name">
			<?php if ($params->get('link_titles') && $params->get('access-view')) : ?>
				<a href="<?php echo JRoute::_(Perfectpics_productsHelperRoute::getArticleRoute($this->item->slug, $this->item->catid)); ?>" itemprop="url">
				<?php echo $this->escape($this->item->product_name); ?></a>
			<?php else : ?>
				<?php echo $this->escape($this->item->product_name); ?>
			<?php endif; ?>
		</h2>
		<?php endif; ?>
...
<?php if ($params->get('show_readmore') && $this->item->readmore) :
	if ($params->get('access-view')) :
		$link = JRoute::_(Perfectpics_productsHelperRoute::getPerfectPics_ProductRoute($this->item->slug, $this->item->catid));
	else :
		$menu = JFactory::getApplication()->getMenu();
		$active = $menu->getActive();
		$this->itemId = $active->id;
		$link1 = JRoute::_('index.php?option=com_users&view=login&Itemid=' . $this->itemId);
		$returnURL = JRoute::_(Perfectpics_productsHelperRoute::getPerfectPics_ProductRoute($this->item->slug, $this->item->catid));
		$link = new JUri($link1);
	...
	<?php if (!$params->get('access-view')) :
		echo JText::_('COM_PERFECTPICS_PRODUCTS_REGISTER_TO_READ_MORE');
	elseif ($readmore = $this->item->alternative_readmore) :
		echo $readmore;
		if ($params->get('show_readmore_title', 0) != 0) :
		echo JHtml::_('string.truncate', ($this->item->product_name), $params->get('readmore_limit'));
		endif;
	elseif ($params->get('show_readmore_title', 0) == 0) :
		echo JText::sprintf('COM_PERFECTPICS_PRODUCTS_READ_MORE_TITLE');
	else :
		echo JText::_('COM_PERFECTPICS_PRODUCTS_READ_MORE');
		echo JHtml::_('string.truncate', ($this->item->product_name), $params->get('readmore_limit'));
	endif; ?>
