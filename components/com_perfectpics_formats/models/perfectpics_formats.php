...
/**
 * Item Model for perfectpics_formats.
 *
 * @package     Perfectpics_formats
 * @subpackage  Models
 */
class Perfectpics_formatsModelPerfectPics_Formats extends JModelAdmin
{
	/**
	 * @var        string    The prefix to use with controller messages.
	 * @since   1.6
	 */
	protected $text_prefix = 'COM_PERFECTPICS_FORMATS';
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_perfectpics_formats.perfectpics_formats';
	 ...
	 */
	public function getTable($type = 'PerfectPics_Formats', $prefix = 'Perfectpics_formatsTable', $config = array())
	{
	...
	{
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('PerfectPics_Formats.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$query = $db->getQuery(true)
					->select('a.*')
					->from('#__perfectpics_format AS a');
				...
				if (empty($data))
				{
					JError::raiseError(404, JText::_('COM_PERFECTPICS_FORMATS_ERROR_FEED_NOT_FOUND'));
				}
